...
public final class EntityReseConfig {
    ...
    public static synchronized void load() {
        ...
        try {
            ...
            ResetModule.LOGGER.error("Failed to load entity reset config", exception);
            closeConfig();
        }
    }
    ...
    private static void replaceRulesInMemory(List<String> values) {
        ...
        for (String raw : values) {
            ...
            if (parsed == null) {
                ResetModule.LOGGER.warn("Removed invalid Entity Rule: {}", raw);
                continue;
            }
    ...
    public static synchronized boolean saveRuleAuthoritative(
        ...
        try {
            ...
            ResetModule.LOGGER.error("Failed to save entity reset rule for {}", entityId, throwable);
            return false;
        }
    }
    ...
    public static synchronized boolean removeRuleAuthoritative(String entityId) {
        ...
        try {
            ...
            replaceRulesInMemory(before);
            if (configData != null) configData.set("general.rules", new ArrayList<>(before));
            ResetModule.LOGGER.error("Failed to remove entity reset rule for {}", entityId, throwable);
            return false;
        }
    }
